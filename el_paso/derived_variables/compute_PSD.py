...
from el_paso.physics import pfunc

def compute_PSD(product, psd_var):
    ...
    pct = pfunc(energy_var.data_content, species_char)  # Relativistic energy for electrons
    psd_data = (1e3 / 2.997e10) * flux_var.data_content / (pct[:, :, np.newaxis] ** 2)

    psd_var.data_content = psd_data
    psd_var.metadata.unit = (u.m * u.kg * u.m / u.s)**(-3)