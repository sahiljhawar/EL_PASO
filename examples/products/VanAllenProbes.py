...
class RBSPICE_tofxeh(Product):
    ...
    def __init__(self, satellite_str, save_data_dir, download_data_dir, magnetic_field_model='T89'):
        """Initializes the Arase HEP Realtime product."""
        super().__init__(name='RBSPICE_tofxeh')
        ...
        self.default_save_standard = DataorgPMF(mission='RBSP', source=satellite_str, instrument='RBSPICE-tofxeh-l3',
                                                save_text_segments=[save_data_dir, satellite_str,
                                                                    'n4', '4', 'T89', 'ver4'], varnames=varnames)

        # Part 1: specify source files to extract variables

        time_var = TimeVariable(name_or_column_in_file='Epoch', original_unit=u.epoch_tt2000)
        ...
        variables_to_extract =  {
            ...
            'PA_local_FPDU': Variable(name_or_column_in_file='PA_Midpoint', original_unit=u.deg, target_unit='',
                     standard_name='PA_local_FPDU', time_bin_method=TimeBinMethod.NoBinning, time_variable=None),
            'xGEO': Variable(name_or_column_in_file='Position', standard_name='xGEO',
                     original_unit=u.km, target_unit=u.RE, time_bin_method=TimeBinMethod.NanMedian, time_variable=time_var)
        }
        ...
        self.derived_variables = {
            ...
            'B_local': DerivedVariable(standard_name=f'B_local_{magnetic_field_model}', target_unit=u.nT, time_variable=time_var)
        }
    ...
    def process_after_binning(self) -> None:
        ...
        # fold_pitch_angles_and_flux_variable(self.get_variable_by_workspace_name('FPDU'),
        #                                     self.get_variable_by_workspace_name('PA_local_FPDU'))