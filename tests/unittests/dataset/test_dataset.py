...
# @pytest.fixture(params=["nc", "h5", "cdf", "mat"])
@pytest.fixture(params=["nc"])
def mock_dataset(request, tmp_path: Path) -> DataSet:
...
@pytest.mark.basic
def test_init_datetime_timezone(mock_dataset: DataSet):
...
def test_computed_invv_variable(mock_dataset: DataSet):
    """Test computed InvV variable."""

    mock_dataset.InvK = np.array([[1.0, 2.0]])  # ty:ignore[unresolved-attribute]
    mock_dataset.InvMu = np.array([[0.1, 0.2], [0.3, 0.4]])  # ty:ignore[unresolved-attribute]
...
def test_computed_p_variable(mock_dataset: DataSet):
    """Test computed P variable."""

    mock_dataset.MLT = np.array([0.0, 6.0, 12.0, 18.0])  # ty:ignore[unresolved-attribute]
...
def test_load_variable_real_file(mock_dataset: DataSet):

    mock_dataset._load_variable("alpha_local")

    assert hasattr(mock_dataset, "alpha_local"), "Dataset should have 'alpha_local' attribute after loading."
...
# def test_enable_dict_loading_mode(mock_dataset: DataSet):
#     """Test that default file loading mode raises RuntimeError on update_from_dict."""
...
#     ds = DataSet(satellite="GOESSecondary", instrument="MAGED", mfm=MfmEnum.T89)
...
#     assert "DataSet" in repr(dict_dataset)
...
#     assert "DataSet" in str(dict_dataset)
...
#                 dataset1.datetime = [datetime(2023, 1, 15, tzinfo=timezone.utc)]

#                 dataset2.Flux = np.array([[1.0, 2.0, 3.0]])
#                 dataset2.time = np.array([738000.0])
#                 dataset2.datetime = [datetime(2023, 1, 15, tzinfo=timezone.utc)]
...
#     test_datetime_list = [datetime(2023, 1, 15, tzinfo=timezone.utc)]
...
# @pytest.fixture
# def mock_dataset_nc(mocker) -> DataSet:
...
# def test_get_satellite_name_nc(mock_dataset_nc: DataSet):
#     """Test get_satellite_name method."""
#     assert mock_dataset_nc.get_satellite_name() == "arase"


# def test_get_satellite_and_instrument_name_nc(mock_dataset_nc: DataSet):
#     """Test get_satellite_and_instrument_name method."""
#     assert mock_dataset_nc.get_satellite_and_instrument_name() == "arase_XEP"


# def test_get_print_name_nc(mock_dataset_nc: DataSet):
#     """Test get_print_name method."""
#     assert mock_dataset_nc.get_print_name() == "arase XEP"


# def test_getattr_with_valid_variable_nc(mock_dataset_nc: DataSet):
#     """Test __getattr__ with a valid variable."""
...
# def test_getattr_with_invalid_variable_nc(mock_dataset_nc: DataSet):
#     """Test __getattr__ with an invalid variable."""
#     with pytest.raises(AttributeError):
#         _ = mock_dataset_nc.NonExistentAttribute


# def test_getattr_with_similar_variable_nc(mock_dataset_nc: DataSet):
#     """Test __getattr__ suggests similar variable name."""
#     with pytest.raises(AttributeError) as e:
...
# def test_computed_invv_variable_nc(mock_dataset_nc: DataSet):
#     """Test computed InvV variable."""
...
# def test_computed_p_variable_nc(mock_dataset_nc: DataSet):
#     """Test computed P variable."""
...
# def test_create_date_list_monthly_nc(mock_dataset_nc: DataSet):
...
# def test_create_date_list_daily_nc(mock_dataset_nc: DataSet):
...
# def test_file_name_stem_generation_nc(mock_dataset_nc: DataSet):
#     """Test that file name stem is generated correctly."""
#     assert mock_dataset_nc._create_file_name_stem() == "arase_XEP_"


# def test_file_path_stem_dataserver_nc(mock_dataset_nc: DataSet):
...
# def test_invalid_cadence_raises_nc(mock_dataset_nc: DataSet):
...
# def test_invalid_folder_type_raises_nc(mock_dataset_nc: DataSet):
...
# def test_get_var_method_nc(mock_dataset_nc: DataSet):
...
#     end_time = datetime(2025, 4, 30, tzinfo=timezone.utc)
...
# def test_all_variables_in_dir_nc(mock_dataset_nc: DataSet):
#     vars = [
#         "datetime",
#         "time",
#         "energy_channels",
...
# def test_load_all_variables_nc(mock_dataset_nc: DataSet):
...
# def test_load_variable_netcdf_caches_file_reads(mock_dataset_nc: DataSet):
#     """Repeated NetCDF loads should reuse the parsed monthly file content."""
#     datasets = {
#         "time": np.array([datetime(2026, 3, 15, tzinfo=timezone.utc).timestamp()], dtype=np.int64),
#         "flux/alpha_local": np.array([[0.1, 0.2, 0.3]]),
#         "flux/FEDU": np.array([[1.0, 2.0, 3.0]]),
#     }

#     with mock.patch(
#         "swvo.io.DataSet.DataSet.read_all_datasets_netcdf",
...
# def test_get_loaded_variables_includes_computed_variables_nc(mock_dataset_nc: DataSet):
#     """Computed variables should be tracked once accessed."""
#     _ = mock_dataset_nc.P
...
#         datetime(2013, 1, 2, tzinfo=timezone.utc),
