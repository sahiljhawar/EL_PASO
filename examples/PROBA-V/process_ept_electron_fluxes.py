...
def process_ept_electron_fluxes(
    ...
    extraction_infos = [
        ...
        ep.ExtractionInfo(result_key="millisecond", name_or_column="mS", unit=u.dimensionless_unscaled, np_dtype=np.int32),
    ...
    flux_data = np.stack([variables["ch0"].get_data(), variables["ch1"].get_data(), variables["ch2"].get_data(),
                          variables["ch3"].get_data(), variables["ch4"].get_data(), variables["ch5"].get_data()]).T
    ...
    epoch_datetime = [datetime(y, m, d, h, mi, s, int(ms), tzinfo=timezone.utc) for (y, m, d, h, mi, s, ms) in
                      zip(variables["year"].get_data(), variables["month"].get_data(), variables["day"].get_data(),
                          variables["hour"].get_data(), variables["minute"].get_data(), variables["second"].get_data(),
                          variables["millisecond"].get_data().astype(np.int32)*1e3, strict=True)]
    ...
    xsph_arr = np.stack(
        (variables["rad"].get_data(ep.units.RE), variables["lat"].get_data(u.degree), variables["lon"].get_data(u.degree))
    ).T.astype(np.float64)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process EPT electron fulx data."
    )
    parser.add_argument(
        "--start_time",
        type=str,
