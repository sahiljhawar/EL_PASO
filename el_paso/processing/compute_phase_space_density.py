...
def compute_phase_space_density(
    ...
    Returns:
        ep.Variable: A new Variable object containing the computed Phase Space
            Density (PSD) data, with unit $(c / MeV \cdot cm)^{3}$.

    Notes:
        - The constant `(1e3 / 2.997e10)` for each psd value while computation which accounts for the speed of light.
    """
    ...
    var = ep.Variable(data=psd_data, original_unit=typing.cast("u.UnitBase", ep.units.psd_natural_unit))
