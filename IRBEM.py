...
class MagFields:
    ...
    def make_lstar_shell_splitting(self, X, maginput, alpha_in):
        ...
        if self.TMI: print("Running IRBEM-LIB make_lstar_shell_splitting")

        self._irbem_obj.make_lstar_shell_splitting1_(ctypes.byref(ntime), ctypes.byref(Nipa), ctypes.byref(self.kext),
                                                     ctypes.byref(self.options), ctypes.byref(self.sysaxes),
                                                     ctypes.byref(iyear),
                                                     ctypes.byref(idoy), ctypes.byref(ut), ctypes.byref(x1),
                                                     ctypes.byref(x2), ctypes.byref(x3), ctypes.byref(alpha),
    ...
    def drift_bounce_orbit(self, X, maginput, alpha=90, R0=1):
        ...
        self._irbem_obj.drift_bounce_orbit2_1_(ctypes.byref(self.kext), ctypes.byref(self.options), \
                                               ...
                                               ctypes.byref(R0), ctypes.byref(lm), ctypes.byref(lstar),
                                               ctypes.byref(blocal), \
                                               ctypes.byref(bmin), ctypes.byref(bmirr), ctypes.byref(xj),
                                               ctypes.byref(posit), \
        ...
        self.drift_bounce_orbit_output = {'Lm': lm.value, 'blocal': np.array(blocal),
                                          'bmin': bmin.value, 'bmirr': bmirr.value, 'lstar': lstar.value,
                                          'xj': xj.value,
