...
class AraseXEP(Product):
    ...
    def __init__(self, save_data_dir, download_data_dir, irbem_lib_path, magnetic_field_model='T89'):
        ...
        variables_to_extract_flux =  {
            'Epoch': time_var_flux, 
            'Energy_FEDO': Variable(name_or_column_in_file='FEDO_SSD_Energy', standard_name='Energy_FEDO',
                     original_unit=u.eV, target_unit=u.MeV, time_variable=None),
            'FEDO': Variable(name_or_column_in_file='FEDO_SSD', standard_name='FEDO',
                     original_unit='', time_bin_method=TimeBinMethod.NanMedian, time_variable=time_var_flux),
            'PA_local_FPDU': Variable(standard_name='PA_local', time_variable=None),
        }

        data_path_stem = f"{download_data_dir}/arase/xep/l2/omniflux/YYYY/"
        file_name_stem = " erg_xep_l2_omniflux_YYYYMMDD*.cdf"

        source_file_flux = SourceFile(
            download_url="https://ergsc.isee.nagoya-u.ac.jp/data/ergsc/satellite/erg/xep/l2/omniflux/YYYY/MM/",
