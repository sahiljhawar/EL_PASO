...
from el_paso.saving_strategy import ConsistencyCheck
...
def test_consistency_time_wrong():
    ...
    with pytest.raises(ValueError, match="Time length missmatch!"):
...
def test_consistency_energy_wrong():
    ...
    with pytest.raises(ValueError, match="Energy length missmatch!"):
...
def test_consistency_pitch_angle_wrong():
    ...
    with pytest.raises(ValueError, match="Pitch angle length missmatch!"):
        consistency_check.check_pitch_angle_size(ENERGY_LEN+1, "call3")
